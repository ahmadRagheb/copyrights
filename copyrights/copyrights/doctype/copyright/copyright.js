...
frappe.ui.form.on("Copyright", {
	...
	onload: function(frm) {
		var so = frappe.meta.get_docfield("Copyright", "sales_order");
		...
		frm.set_query("user", "users", function() {
			return {
				query:"copyrights.copyrights.doctype.copyright.copyright.get_users_for_project"
			}
		});
	...
	refresh: function(frm) {
		if(frm.doc.__islocal) {
			...
			frm.add_web_link("/copyrights?copyright=" + encodeURIComponent(frm.doc.name));

			if(frappe.model.can_read("Task")) {
				frm.add_custom_button(__("Gantt Chart"), function() {
					frappe.route_options = {"Copyright": frm.doc.name};
